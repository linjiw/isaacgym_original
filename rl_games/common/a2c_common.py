...
class A2CBase(BaseAlgorithm):
    ...
    def __init__(self, base_name, params, created_env = None):
        print('A2CBase init')
        self.config = config = params['config']
        pbt_str = ''
        self.population_based_training = config.get('population_based_training', False)
        ...
        if created_env is not None:
            print('created_env is provided, not creating vec env')
            self.vec_env = created_env
            self.env_info = self.vec_env.get_env_info()
        elif self.env_info is None:
            print('env_info is not provided, creating vec env')
            self.vec_env = vecenv.create_vec_env(self.env_name, self.num_actors, **self.env_config)
            self.env_info = self.vec_env.get_env_info()
        else:
            print('env_info is provided, not creating vec env')
            self.vec_env = config.get('vec_env', None)
        ...
        self.batch_size = self.horizon_length * self.num_actors * self.num_agents # 32 * 8 * 1 = 256 32 *512 *1 = 16384
        # self.batch_size = self.horizon_length * 512
        # print(f"batch_size {self.batch_size}")
        print(f"batch_size {self.batch_size}")
        print(f"horizon_length {self.horizon_length}")
        print(f"num_actors {self.num_actors}")
        ...
        self.minibatch_size_per_env = self.config.get('minibatch_size_per_env', 0)
        self.minibatch_size = self.config.get('minibatch_size', self.num_actors * self.minibatch_size_per_env)
        print(f"minibatch_size {self.minibatch_size}")
        print(f"minibatch_size_per_env {self.minibatch_size_per_env}")
    ...
    def play_steps(self):
        ...
        total_dones = 0
        step_time = 0.0
        # change_env_freq = self.config.get('change_env_freq', 0)
        change_env_freq = self.horizon_length // 4
        map_order = 0
        for n in range(self.horizon_length):
            ...
            successes = sum(infos["success"])
            done_infos = sum(self.dones)
        ...
        if total_dones != 0:
            print(f'total_successes: {total_successes}; total_dones: {total_dones}; success rate: {total_successes / total_dones}')
            wandb.log({f"PAIR/success_rate": total_successes / total_dones * 100})
        else:
            print(f'total_successes: {total_successes}; total_dones: {total_dones}; success rate: 0.0')
            wandb.log({f"PAIR/success_rate": 0.0})
...
class DiscreteA2CBase(A2CBase):
    ...
    def train(self):
        ...
        while True:
            ...
            # calculate time for one epoch and calculate total estimate time and end time
...
class ContinuousA2CBase(A2CBase):
    ...
    def __init__(self, base_name, params, created_env=None):
        A2CBase.__init__(self, base_name, params, created_env)
        print(f"ContinuousA2CBase init")
        self.is_discrete = False
        action_space = self.env_info['action_space']
        self.actions_num = action_space.shape[0]
        self.bounds_loss_coef = self.config.get('bounds_loss_coef', None)
        print(f"self.max_epochs: {self.max_epochs}")
        self.clip_actions = self.config.get('clip_actions', True)
    ...
    def train(self):
        ...
        while True:
            ...
            # if epoch_num_real != epoch_num:
            #     epoch_num = epoch_num_real
            #     print(f"epoch_num_real: {epoch_num_real}")

            step_time, play_time, update_time, sum_time, a_losses, c_losses, b_losses, entropies, kls, last_lr, lr_mul = self.train_epoch()
            total_time += sum_time
            frame = self.frame // self.num_agents
            print(f"epoch_num_real: {epoch_num_real} epoch_num: {epoch_num}")
            print(f"time for 1 epoch: {sum_time}")
            print(f"total estimate time in hours: {sum_time * (self.max_epochs - epoch_num)/ 3600}")
            print(f"end time: {datetime.now() + timedelta(seconds=sum_time * (self.max_epochs - epoch_num))}")
            ...
            if self.rank == 0:
                ...
                if epoch_num_real >= self.max_epochs:
                    if self.game_rewards.current_size == 0:
                        print('WARNING: Max epochs reached before any env terminated at least once')
                        mean_rewards = -np.inf
